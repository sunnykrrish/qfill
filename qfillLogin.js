...
const {
  ...
  POLL_INTERVAL_MS = '2000',     // sleep between checks, measured *after* the page has finished loading
  ROW_WAIT_MS = '8000',          // how long to let Gmail render the inbox before deciding no mail is there
...
const ROW_WAIT_TIMEOUT = Number(ROW_WAIT_MS);

// Gmail is a heavy SPA: `domcontentloaded` fires on the initial shell, well
// before the inbox/search results are actually fetched and rendered. Wait for
// the network to settle first so we're checking a page that has genuinely
// finished loading, not one that's mid-render.
async function waitForGmailSettled(page) {
  await page.waitForLoadState('networkidle', { timeout: ROW_WAIT_TIMEOUT }).catch(() => {});
}

async function quizRowPresent(page) {
  await waitForGmailSettled(page);
  try { await page.locator('tr.zA').first().waitFor({ timeout: ROW_WAIT_TIMEOUT }); return true; }
  catch { return false; }
}
...
async function pollGmailForFormUrl(page) {
  ...
  let found = await quizRowPresent(page);
  while (!found && Date.now() <= deadline) {
    await sleep(interval);
    await page.reload({ waitUntil: 'domcontentloaded' });
    found = await quizRowPresent(page);
  }
  if (!found) return { url: null };

  // Open the newest match and extract the link (unwrapping Gmail's redirect).
